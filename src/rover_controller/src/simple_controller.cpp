#include "rover_controller/simple_controller.hpp"
#include <Eigen/Geometry>
#include <tf2/LinearMath/Quaternion.h>

using std::placeholders::_1;

SimpleController::SimpleController(const std::string& name)
    : Node(name),
      wheel_radius_(0.033),
      wheel_separation_(0.17),
      left_wheel_prev_pos_(0.0),
      right_wheel_prev_pos_(0.0),
      left_front_wheel_prev_pos_(0.0),
      left_back_wheel_prev_pos_(0.0),
      right_front_wheel_prev_pos_(0.0),
      right_back_wheel_prev_pos_(0.0),
      x_(0.0),
      y_(0.0),
      theta_(0.0)
{
    // Parametreleri tanımla
    declare_parameter("wheel_radius", 0.033);
    declare_parameter("wheel_separation", 0.17);
    wheel_radius_ = get_parameter("wheel_radius").as_double();
    wheel_separation_ = get_parameter("wheel_separation").as_double();

    RCLCPP_INFO_STREAM(get_logger(), "Using wheel radius: " << wheel_radius_);
    RCLCPP_INFO_STREAM(get_logger(), "Using wheel separation: " << wheel_separation_);

    // Publisher ve Subscriber'ları oluştur
    wheel_cmd_pub_ = create_publisher<std_msgs::msg::Float64MultiArray>("/simple_velocity_controller/commands", 10);
    vel_sub_ = create_subscription<geometry_msgs::msg::TwistStamped>(
        "/rover_controller/cmd_vel", 10, std::bind(&SimpleController::velCallback, this, _1));
    joint_sub_ = create_subscription<sensor_msgs::msg::JointState>(
        "/joint_states", 10, std::bind(&SimpleController::jointCallback, this, _1));
    odom_pub_ = create_publisher<nav_msgs::msg::Odometry>("/rover_controller/odom", 10);

    // Hız dönüşüm matrisi
    speed_conversion_ << wheel_radius_ / 2, wheel_radius_ / 2,
                         wheel_radius_ / wheel_separation_, -wheel_radius_ / wheel_separation_;
    RCLCPP_INFO_STREAM(get_logger(), "The conversion matrix is:\n" << speed_conversion_);

    // Odometry mesajını doldur
    odom_msg_.header.frame_id = "odom";
    odom_msg_.child_frame_id = "base_footprint";
    odom_msg_.pose.pose.orientation.x = 0.0;
    odom_msg_.pose.pose.orientation.y = 0.0;
    odom_msg_.pose.pose.orientation.z = 0.0;
    odom_msg_.pose.pose.orientation.w = 1.0;

    // TF yayıncısını oluştur
    transform_broadcaster_ = std::make_unique<tf2_ros::TransformBroadcaster>(*this);
    transform_stamped_.header.frame_id = "odom";
    transform_stamped_.child_frame_id = "base_footprint";

    // Zamanı başlat
    prev_time_ = get_clock()->now();
}

void SimpleController::velCallback(const geometry_msgs::msg::TwistStamped &msg)
{
    // Diferansiyel kinematik modeli uygula
    Eigen::Vector2d robot_speed(msg.twist.linear.x, msg.twist.angular.z);
    Eigen::Vector2d wheel_speed = speed_conversion_.inverse() * robot_speed;

    // Tekerlek hızlarını yayınla
    std_msgs::msg::Float64MultiArray wheel_speed_msg;
    wheel_speed_msg.data.push_back(wheel_speed.coeff(1)); // Sağ tekerlek
    wheel_speed_msg.data.push_back(wheel_speed.coeff(0)); // Sol tekerlek
    wheel_cmd_pub_->publish(wheel_speed_msg);
}

void SimpleController::jointCallback(const sensor_msgs::msg::JointState &state)
{
    // Tekerlek pozisyon değişikliklerini hesapla
    if (state.position.size() < 4) {
        RCLCPP_WARN(get_logger(), "JointState message does not contain enough positions.");
        return;
    }

    // Her tekerlek için pozisyon değişikliklerini hesapla
    double dp_left_front = state.position.at(2) - left_front_wheel_prev_pos_;
    double dp_left_back = state.position.at(3) - left_back_wheel_prev_pos_;
    double dp_right_front = state.position.at(0) - right_front_wheel_prev_pos_;
    double dp_right_back = state.position.at(1) - right_back_wheel_prev_pos_;

    // Sol ve sağ tekerleklerin ortalama pozisyon değişikliklerini hesapla
    double dp_left = (dp_left_front + dp_left_back) / 2.0;
    double dp_right = (dp_right_front + dp_right_back) / 2.0;

    // Zaman farkını hesapla
    rclcpp::Time msg_time = state.header.stamp;
    rclcpp::Duration dt = msg_time - prev_time_;

    if (dt.seconds() <= 0) {
        RCLCPP_WARN(get_logger(), "Invalid time difference detected.");
        return;
    }

    // Önceki pozisyonları güncelle
    left_front_wheel_prev_pos_ = state.position.at(2);
    left_back_wheel_prev_pos_ = state.position.at(3);
    right_front_wheel_prev_pos_ = state.position.at(0);
    right_back_wheel_prev_pos_ = state.position.at(1);
    prev_time_ = state.header.stamp;

    // Tekerlek hızlarını hesapla
    double fi_left = dp_left / dt.seconds();
    double fi_right = dp_right / dt.seconds();

    // Lineer ve açısal hızları hesapla
    double linear = (wheel_radius_ * fi_right + wheel_radius_ * fi_left) / 2.0;
    double angular = (wheel_radius_ * fi_right - wheel_radius_ * fi_left) / wheel_separation_;

    // Pozisyon güncellemesi
    double d_s = (wheel_radius_ * dp_right + wheel_radius_ * dp_left) / 2.0;
    double d_theta = (wheel_radius_ * dp_right - wheel_radius_ * dp_left) / wheel_separation_;
    theta_ += d_theta;
    x_ += d_s * cos(theta_);
    y_ += d_s * sin(theta_);

    // Odometry mesajını oluştur ve yayınla
    tf2::Quaternion q;
    q.setRPY(0, 0, theta_);
    odom_msg_.header.stamp = get_clock()->now();
    odom_msg_.pose.pose.position.x = x_;
    odom_msg_.pose.pose.position.y = y_;
    odom_msg_.pose.pose.orientation.x = q.getX();
    odom_msg_.pose.pose.orientation.y = q.getY();
    odom_msg_.pose.pose.orientation.z = q.getZ();
    odom_msg_.pose.pose.orientation.w = q.getW();
    odom_msg_.twist.twist.linear.x = linear;
    odom_msg_.twist.twist.angular.z = angular;
    odom_pub_->publish(odom_msg_);

    // TF mesajını oluştur ve yayınla
    transform_stamped_.transform.translation.x = x_;
    transform_stamped_.transform.translation.y = y_;
    transform_stamped_.transform.rotation.x = q.getX();
    transform_stamped_.transform.rotation.y = q.getY();
    transform_stamped_.transform.rotation.z = q.getZ();
    transform_stamped_.transform.rotation.w = q.getW();
    transform_stamped_.header.stamp = get_clock()->now();
    transform_broadcaster_->sendTransform(transform_stamped_);
}

int main(int argc, char* argv[])
{
    rclcpp::init(argc, argv);
    auto node = std::make_shared<SimpleController>("simple_controller");
    rclcpp::spin(node);
    rclcpp::shutdown();
    return 0;
}
...
class SimpleController(Node):
    ...
    def __init__(self):
        ...
        # Tekerleklerin önceki pozisyonları
        self.left_front_wheel_prev_pos_ = 0.0
        self.right_front_wheel_prev_pos_ = 0.0
        self.left_back_wheel_prev_pos_ = 0.0
        self.right_back_wheel_prev_pos_ = 0.0

        # Robotun başlangıç pozisyonu
        self.x_ = 0.0
        self.y_ = 0.0
        self.theta_ = 0.0
        ...
        self.vel_sub_ = self.create_subscription(TwistStamped, "rover_controller/cmd_vel", self.velCallback, 10)
        self.joint_sub_ = self.create_subscription(JointState, "joint_states", self.jointCallback, 10)
        self.odom_pub_ = self.create_publisher(Odometry, "rover_controller/odom", 10)
    ...
    def jointCallback(self, msg):
        ...
        dp_left_front = msg.position[2] - self.left_front_wheel_prev_pos_
        dp_right_front = msg.position[0] - self.right_front_wheel_prev_pos_
        dp_left_back = msg.position[3] - self.left_back_wheel_prev_pos_
        dp_right_back = msg.position[1] - self.right_back_wheel_prev_pos_

        # Ortalama pozisyon değişiklikleri
        dp_left = (dp_left_front + dp_left_back) / 2
        dp_right = (dp_right_front + dp_right_back) / 2

        # Zaman farkını hesapla
        dt = Time.from_msg(msg.header.stamp) - self.prev_time_

        # Önceki pozisyonları güncelle
        self.left_front_wheel_prev_pos_ = msg.position[2]
        self.right_front_wheel_prev_pos_ = msg.position[0]
        self.left_back_wheel_prev_pos_ = msg.position[3]
        self.right_back_wheel_prev_pos_ = msg.position[1]
